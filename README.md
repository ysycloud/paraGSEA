...
## VII. Using Problem
1. When we get a new profile file keeps in correct format with a ‘gctx’ or ‘gct’ suffix to analysis, we must generate some reference data first to facilitate the main follow-up work of C Tools.Nowdays, the ‘rhd’ and ‘chd’ structs have been splited from some new datasets of LINCS to be separate text files. As for this kind of datasets, we cannot make sure the value of each field just through the ‘gctx’ files and then we cannot extract certain profiles from these datasets. Obviously, we also cannot get reference data from these new datasets. If you want to get the whole profile information in “Quick search” or “Clustering profiles” part, please search the text files it provided by yourselves. Based on this, we strongly encourage you to use ‘gct’ files or ‘gctx’ files with the combinational information of ‘rhd’ and ‘chd’ structs.
